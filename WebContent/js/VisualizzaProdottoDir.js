...
function compilaForm() {
	...
	$.ajax({
		...
		success: function(xml) {
			...
			var isPubblico, statoProdotto, urlfile;
			
			idProprietario=$(xml).find('ID_Proprietario').text();
			proprietarioObj=$(elenco_ricercatori_completo).find("option[value='"+idProprietario+"']");
			...
			$(xml).find("Id_Ricercatori").find("Id").each(function(){
				id_ricercatori[i]=parseInt($(this).text());
				i++;
				if(id_ricercatori[i-1]==parseInt(idProprietario)){
			...
			if(statoProdotto!='draft' && statoProdotto!='completo') { //PRODOTTO VALIDATO
				if(isPubblico=='0') {
					if($("#ID_CurrentUser").val()=='0') { //Se il prodotto non è pubblico, gli utenti esterni non possono visualizzarlo
						...
						createFields();
						disabilitaCampi();	
						$(proprietarioObj).appendTo("#autore_proprietario");
...
function modifica(){
	...
	$.ajax({
		...
		data: dataParameter+"&Stato="+stato+"&ID_Proprietario="+idProprietario,
		dataType: "text",
		success: function(msg){
			if(msg=='OK') {
...
function abilitaCampi() {
	...
	$(elenco_ricercatori_completo).find("option[value='"+idProprietario+"']").remove();
	$("#authors_select option[value='"+idProprietario+"']").remove();
	$("#ID_Ricercatore option[value='"+idProprietario+"']").remove();
	refreshAutori();
	createTooltips();
}
function disabilitaCampi() {
	...
	$("#autore_proprietario").prop('disabled', 'true');
...
function refreshAutori() {
	...
	$("#authors_select option[value='"+$("ID_Proprietario").val()+"']").remove();
}
function aggiungiAutore() {
	...
	if(parseInt($(selectedAutore).val())==idProprietario){
		showModal("Inserimento non consentito", "Non puoi inserire il proprietario del prodotto negli autori");
		return;
	}
